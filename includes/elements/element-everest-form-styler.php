...
class Element_Everest_Form_Styler extends Element
{
	...
	{
		return esc_html__('Everest form Styler', 'bricks');
	}
	...
	public function set_controls()
	{
		 ...
		 * Submit button
		 *****************************/
		$selector = '.everest-forms button[type="submit"]';
	...
	public function render()
{
    ...
    // Function to get value from settings
    $get_value = function($array, $key, $default = null) {
        return isset($array[$key]) ? $array[$key] : $default;
    };

    $source = $get_value($settings, 'source_type', 'static');
    $ef_form_id = ($source == 'static') ? $get_value($settings, 'cf_form', 'none') : get_post_meta($this->post_id, $get_value($settings, 'form_id', false), true);

    if ($ef_form_id == 'none') {
        echo $this->render_element_placeholder(['title' => esc_html__('Select a form.', 'bricks')]);
    } elseif (empty($ef_form_id) || $ef_form_id === false) {
        ...
        if ($show_form_title == 'yes') {
            ...
            $title = get_post_field('post_title', $ef_form_id);
            echo "<{$tag} class=\"everest-form-title\">" . wp_kses_post($title) . "</{$tag}>";
        }

        echo do_shortcode('[everest_form id="' . $ef_form_id . '"]');
